...
export const Dashboard = () => {
  ...
  const atCap = usage ? usage.usage.endpoints >= usage.limits.maxEndpoints : false;
  const usageLabel = usage
    ? `${usage.plan} plan · ${usage.usage.endpoints}/${usage.limits.maxEndpoints} monitors${atCap ? ' · limit reached' : ''}`
    : null;
          ...
          <PageHeader
            ...
            subtitle={
              <>
                {subtitle}
                {usageLabel && (
                  <span style={{ color: atCap ? '#f59e0b' : 'var(--text-muted)', marginLeft: 10, fontSize: 12 }}>
                    {usageLabel}
                  </span>
                )}
              </>
